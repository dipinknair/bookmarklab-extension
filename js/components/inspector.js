...
function renderBookmarkInspector(containerEl, node) {
  ...
  const stagedBadge = `<div style="background:rgba(99,102,241,0.1); border:1px solid rgba(99,102,241,0.25); border-radius:6px; padding:6px 10px; font-size:0.73rem; color:#818cf8; margin-bottom:12px; display:flex; align-items:center; gap:6px;">
    ...
    ${escapeHTML(t('inspectorStagedNotice', 'Edits are staged — click Sync Bookmarks in the toolbar to apply.'))}
   </div>`;
  ...
  containerEl.innerHTML = `
    <div class="inspector-form">
      ...
      <div class="form-group">
        <label class="form-label">${escapeHTML(t('inspectorLabelTitle', 'Title'))} <span style="font-weight:normal; color:var(--text-muted); font-size:0.75rem;">(Leave empty for icon-only)</span></label>
        <input type="text" id="inp-title" class="form-input" placeholder="Icon-only bookmark" value="${escapeHTML(node.title)}" />
      </div>

      <div class="form-group">
        <label class="form-label">${escapeHTML(t('inspectorLabelUrl', 'URL'))}</label>
        ...
        ${hasChanges ? `
          <button id="btn-strip-this-url" class="btn-sm btn-accent" style="margin-top:6px;">
            ${escapeHTML(t('inspectorStripParams', '⚡ Remove Tracking Query Parameters'))}
          </button>
        ` : ''}
      </div>

      <div class="form-group">
        <label class="form-label">${escapeHTML(t('inspectorLabelFolder', 'Folder Location'))}</label>
